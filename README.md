...
| `--max-watches <n>` | 4 | データ待ち行列の**常駐監視**の同時本数。監視 1 本＝ホストサーバー接続 1 本を占有する |
...
| **セッション設定** | `system`（親システムの参照）/ `sessionType` / `deviceName` / `deviceNameRetry` / `screenSize` / `ccsid`（上書き）/ `enhanced` / `watermark` / `rescueAction` / `transformTo` / `printer`・`pcCommand`（サーバー設定のみ） |
...
"pcCommand": {
  ...
  "allow": ["start https?://.*", "notepad .*"]  // 正規表現・全体一致。省略すると制限なし
}
```
...
- `allow` は**全体一致**で判定します（前方一致にすると `notepad; rm -rf /` のような後置きが素通りするため）。
