...
| | サーバー設定<br>参照・利用 | サーバー設定<br>編集 | 自分の設定 | 5250端末<br>セッション | プリンター<br>セッション | 自動 PDF / 印刷<br>**の設定** | 管理者画面 |
|---|---|---|---|---|---|---|---|
| **個人利用** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅（セッション・ログ） |
| **admin** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **一般ユーザー** | ❌ | ❌ | ✅（自分のみ） | ✅ | ✅ | ❌ | ❌ |
...
- **セッション種別（5250端末 / プリンター）は作成時に決め、以後変更できません。**
...
| **セッション設定** | `system`（親システムの参照）/ `sessionType` / `deviceName` / `screenSize` / `ccsid`（上書き）/ `enhanced` / `printer`（サーバー設定のみ） |
...
- **`session` を指定すれば親システムまで一意に決まります**ので、`system` の併記は不要です。
  ...
  勝つ挙動は採りません。どちらも無く `host` も無ければ `CONNECT_FAILED` です。
- **`system` だけで足りる操作があります。** SQL・IFS・ホスト一覧（ジョブ/オブジェクト/ユーザー）は
  装置名も画面サイズも使わないため、システム参照だけを渡せば動きます。
- **信頼設定（自動 PDF/印刷）を持てるのはサーバー設定のセッションだけ**です。個人設定側は
  `personalSessionSchema` の `.strict()` が `printer` を拒否し、解決時も `srv:` 由来のセッション
  からしか出力設定を供給しません（信頼境界）。
...
- **システム側**に指定できるのは `tls: true`（ポート既定 992）と `ccsid`（930/939/1399 等で DBCS。既定値）。
  **セッション設定側**は `deviceName`、`ccsid`（システムの既定を上書き）、`screenSize`（`"24x80"` /
  `"27x132"`）、`enhanced: true`（拡張 5250 GUI 広告）、`printer`（プリンター種別のみ）。
  `screenSize` と `enhanced` は `sessionType: "display"` のときだけ効きます。
