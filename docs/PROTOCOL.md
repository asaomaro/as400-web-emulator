...
紐づけていない。tn5250 は DBCS 未実装（`valid_terms` の 5555 系 2 件はコメントアウト）で先例にならない。
そのため上表は **PUB400 実機で総当たりして決めた**もので、移植時は実機で追試すること:

| 端末タイプ | 実機の色 | 実機のワイド画面（`STRSEU`） |
|---|---|---|
| `IBM-5555-B01` | モノクロ（4 色） | 24x80 |
| `IBM-5555-C01` | カラー（7 色） | 27x132 |
| `IBM-5555-G01` | モノクロ（4 色） | 24x80 |
...
| 66/68/6A/6C | READ SCREEN TO PRINT 系 | | 72 / 83 | READ IMMEDIATE / ALT |
...
- **SAVE PARTIAL SCREEN（03）は端末に画面を送り返させる要求**で、**パラメータ 5 バイト**が続く
  （実機 SR-OSAKA の QSH は `04 03 00 00 00 00 00`。opcode は `PUT/GET(03)`）。
...
- **ROLL（23）は `方向＋行数(1) 上端(1) 下端(1)`**。
  **上位ビット（`0x80`）が落ちていれば上へ・立っていれば下へ**、行数は下位 5 ビット。
  ⚠ **実機では未確認**（11 画面の国勢調査で 0 件）。根拠は原典 2 実装の一致
  ——tn5250 `session.c`＋`dbuffer.c`（ビットが落ちていれば行数を負にし、負を "Move text up"）と
  tn5250j `Screen5250.rollScreen` のコメント「0 - up / 1 - down」。

### 4.1.1 コマンドのパラメータ長（原典由来）

| コマンド | パラメータ | 出典 |
|---|---|---|
| `WTD`(11) / `WEC`(21) | CC1・CC2 の 2 バイト | 実測・原典 |
| `WEC TO WINDOW`(22) | 開始桁・終了桁 ＋ CC1・CC2 | 実測（SR-OSAKA） |
| `CLEAR UNIT ALTERNATE`(20) | 1 バイト | 実測 |
| **`SAVE PARTIAL SCREEN`(03)** | **5 バイト**＝フラグ・上端行・左端桁・窓の深さ・窓の幅 | tn5250 `session.c`（実測でも 5 バイト。値は全て `00`） |
| **`RESTORE PARTIAL SCREEN`(13)** | **無し**（後続は WTD） | tn5250（1 バイトも読まない） |
| `ROLL`(23) | 3 バイト | tn5250 / tn5250j |
| `READ`系(42/52/82) | CC1・CC2 | 実測 |
| **`READ SCREEN TO PRINT`(66/68/6A/6C)** | **無し** | tn5250（読まずに無視） |
| **`READ IMMEDIATE`(72) / `ALT`(83)** | **無し** | tn5250（72 は実装・83 は無視） |

⚠ **`72` / `83` は本来応答が要る**（原典 tn5250 は 72 で全フィールドを即送信する）。
当方は**応答していない**——実機で届いたことが無く、応答の正しさを確かめられないため。
受理して後続を処理し、届いた事実を警告で残す。

### 4.1.2 保管物（SAVE / RESTORE）の扱い

**ホストは端末が返した積荷をそのまま保管し、復元時にそのまま返す**
（実機 SR-OSAKA で実験して確認——応答の先頭 7 バイトを外すと、
戻ってくるレコードもちょうど 7 バイト短くなった）。
中身はホストにとって不透明なので、**端末は自分が読める形で返せばよい**。
当方は `SAVE SCREEN` / `SAVE PARTIAL SCREEN` のどちらにも
`ESC RESTORE SCREEN(12)` ＋画面を再現する WTD を返す（`12` は局所の退避を戻す目印）。
原典 tn5250 は目印を付けず WTD だけを返す（そのぶん復元時に `12`/`13` は現れない）。
...
- **この境界はフォーマットテーブルとは独立に保持する。** ホストは窓を重ねるとき SOH（§4.2）で
  フォーマットテーブルを消すことがあり（実機 SR-OSAKA の Attn でフィールドが 44 → 2 になる）、
  境界をフィールド一覧から引いていると**画面の中身は変わっていないのに背面の下線が伸びる**。
  境界を捨てるのは CLEAR UNIT・画面サイズ変更（画面の中身ごと消えるとき）と、
  **新しいフィールドの範囲に重なったとき**（同じ場所の描き直し）だけにする。
- この境界は**画面イメージを送り返すとき（READ SCREEN / READ SCREEN EXTENDED）も再現する**——
  空いているフィールド終端桁に通常属性 `20` を置いて送る。§6.3 参照。
...
ホストが**クライアント PC でコマンドを実行させる**仕組み。構造化フィールドではなく、
**WTD の画面データに固定バイト列を埋めて**送ってくる（実機 IBM i 7.x で実測）。

```
11 01 01                          SBA → 行 1 桁 1
27                                属性（非表示）
80 FC D7 C3 D6 40 83 80 A1 80     PCO 標識（"PCO " が読める）
<pause>                           81('a')=待たない / 80=完了を待つ
<command…>                        EBCDIC のコマンド本文（0x40 以上が続く限り）
02 <row> <col> 00                 RA → 残りを NUL 埋め（＝本文の終端。空白詰めではない）
04 52 00 00                       READ MDT FIELDS（ホストは応答を待つ）
```

- 属性込みの 11 バイト `27 80 FC D7 C3 D6 40 83 80 A1 80` が開始標識。
  `27 00 FC D7 C3 D6 40 83 80 82 00` は PC Organizer 終了の標識（xtn5250 由来。**未検証**）。
- 本文は 1 行の桁数を越えても**折返し位置に SBA が入らない**。素直に読み進めてよい。
  実機で確認できた上限は 123 バイト（V7R1 の `PCCMD` 長）。
- クライアントは（実行の可否に関わらず）**AID Enter の Read MDT 応答を返す**。返さないとホストは待ち続ける。
  ホストは実行されたかを検証せず、応答を受けると CLEAR UNIT ＋次画面を送って CL の次行へ進む。
- 同じジョブで **`STRPCO` を先に実行していないと標識は送られてこない**（`STRPCCMD` は無反応で成功する）。
  `STRPCO` 自体はデータストリーム上**何も送らない**ので、クライアント側の実装は不要。
- 画面には出ない（非表示属性）。この画面は未対応クライアント向けに
  「PCO.EXE が活動状態でない」という案内文も含んでおり、対応クライアントは即座に応答するので見えない。

> 実測の詳細と参照実装（tn5250j `tnvt.java` / xtn5250 `XI5250Emulator.java`）との突き合わせは
> `.aidev/works/20260728-strpco-strpccmd/research.md`。
...
| 34 | 00 | |
| 35,36 | model 2 桁（EBCDIC） | 例 "02"→F0 F2。model が 3 桁の 5555 系は**末尾 2 桁**を送る（"C01"→"01"）＝ B01/C01/G01 は Query Reply 上は区別されない。区別しているのは § 2.1 の TERMINAL-TYPE 文字列。 |
| 37 | 02 | キーボード ID（標準） |
| 38,39 | 00 00 | |
...
**フラグレコードは応答を待たない。** ホストが黙って無視するのが正常にあり得る（ATNPGM が既に前面の
ときなど。実機 SR-OSAKA で 2 回目の Attn に受信ゼロを確認。ACS も 2 回目では何も起きない）。
1 回目で窓が出るのはホストが**その後に**画面を送ってくるからで、端末が待つ必要はない。
...
ホストが止まり、画面が変わらずキーボードもロックしたままになる——実機 SR-OSAKA での対照実験では、
...
いずれもホストが**待っている返信**。形式は `save-screen.ts` 参照（0x62 は「カーソル行桁＋全域 1 桁 1 バイト」、
0x64 は「行ごとのバイト列＋区切り `FF`・行末 NUL は切り詰め」、SAVE SCREEN は WTD ストリーム）。

**フィールド終端には閉じ属性 `20` を入れて送る**（その桁が空いている場合のみ。ホストが書いた桁・別の
欄のデータ桁は上書きしない）。5250 のフィールドは開始属性しか持たず終端はフォーマットテーブルの長さで
決まるため、画面イメージだけを送るとホストに「下線がどこで終わるか」が伝わらない。ホストはヘルプ
ウィンドウを出すとき、この応答をそのまま CLEAR UNIT ＋全画面 WTD で描き直して背面を再現するので、
閉じ属性が無いと**背面の下線が入力範囲を越えて行末まで伸び、次行へ回り込む**（ACS は背面がヘルプ前と
まったく変わらない。実機 SR-OSAKA の PDM F1 ヘルプで、閉じ属性を入れるとホストの描き直しが
`(3,32) ATTR 20` 等を含むようになり ACS と一致することを確認）。
...
- **変換表の生成**: ICU の `.ucm`（例 ibm-37 / ibm-930 / ibm-939 / ibm-1399）の SBCS/DBCS セクションから
  ...
  ことで表を一致させられる。
- **930/939 の DBCS 部は CCSID 300**（16684 を使うのは 1399 だけ）。300 の 5 文字は ICU が Unicode 規格寄りに
  割り当てるが、**ACS / jt400 と同じ全角形を正とする**（`codec/ccsid300.ts` が唯一の出所。純 DBCS の CCSID 300 と共用）:
  `4260`→U+FF0D `426A`→U+FFE4 `43A1`→U+FF5E `444A`→U+2015 `447C`→U+2225。
  規格寄りの符号位置（U+2212 等）は East Asian Width が Ambiguous で、欧文等幅フォントが半角で描くため
  **DBCS の桁が 1 桁ずれる**（PDM の F1 ヘルプ「オプション－ヘルプ」で実測）。逆方向はどちらの符号位置からも
  同じバイト対へ寄るので往復は変わらない。
