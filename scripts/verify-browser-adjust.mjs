// 実ブラウザ（web-ui）で SR-OSAKA へ接続し、**ローカル編集キーと FFW の ADJUST（右寄せ）**を検証する。
//
...
//
// 画面は `scripts/build-adjtest-osaka.mjs` が作る ASAOLIB/ADJPGM。
// **最後に Enter を送り、ホストが受け取った値（RPG が `[...]` で返す）まで確かめる**——
// 画面上で右寄せできているだけでは、送信経路（core の内容検証・末尾空白の扱い）を通った保証がない。
//
// 前提: npm run build 済み。`connections.json` に SR-OSAKA と AS01。
...
// connections.json をそのまま使う（AS01=既存装置。ユニーク名は QAUTOVRT 上限に当たるため再利用）
...
const password = crypto.decrypt(cfg.systems.find((s) => s.name === "SR-OSAKA").signon.passwordEnc);
const user = cfg.systems.find((s) => s.name === "SR-OSAKA").signon.user;
...
try {
  ...
  await runCmd("ADDLIBLE ASAOLIB");
  await runCmd("CALL ASAOLIB/ADJPGM");
