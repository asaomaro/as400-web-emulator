// SR-OSAKA で**取得量の上限が実際に効いているか**を、MCP と REST の両方の入口で確かめる。
//
// 単体テストは偽の接続で「要求したブロッキング係数」を固定しているだけなので、
// **実機で本当に速く・小さくなったか**はここで測る。
// 比較の相手は research（`research-sql-cancel-osaka.mjs`）で採った全件取得の実測値:
// 20,000 行 × CHAR(50) で **201 往復 / 1,191,336 バイト / 2,072ms**。
//
// 実行: AS400_PASSWORD=... node scripts/verify-sql-limit-osaka.mjs
...
forApp.systems.find((s) => s.name === "SR-OSAKA").signon = {
...
try {
  ...
  const res = await hostSql({
    system: "srv:" + forApp.systems.find((s) => s.name === "SR-OSAKA").id,
  ...
  const src = { system: "srv:" + forApp.systems.find((s) => s.name === "SR-OSAKA").id };
  const t1 = Date.now();
