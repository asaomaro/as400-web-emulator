...
try {
  ...
  let ok=false;
  for (let a=1;a<=8&&!ok;a++){ await page.click(".card:has-text('AS01') >> button:has-text('接続')"); try{ await page.waitForSelector("input.grid-input",{timeout:15000}); ok=true; }catch{ log(`busy ${a}`); await sleep(11000);} }
