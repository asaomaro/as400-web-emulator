// ASAOLIB の画面プログラムを順に CALL し、遷移する画面をまとめて HTML にする。
// プログラム 1 本につき HTML 1 枚（前後にたどれる履歴版）。
//
// 対象は DSPPGMREF で「表示装置ファイルを参照する」と分かったものだけ
// （scripts/probe-asaolib-refs.mjs の結果）。
//
// 実行: node --env-file=.env scripts/shot-asaolib-screens.mjs <出力ディレクトリ>
import { mkdirSync, writeFileSync } from "node:fs";
import { Client } from "@modelcontextprotocol/sdk/client/index.js";
import { StdioClientTransport } from "@modelcontextprotocol/sdk/client/stdio.js";

const OUTDIR = process.argv[2] ?? "asaolib-screens";
 ...
 * 保存値（`SR-OSAKA`）で開くので、AS400_HOST を必須にすると既定の使い方が壊れる。
 */
...
const client = new Client({ name: "shot-asaolib", version: "0" });
...
async function connect() {
  ...
  // **ライブラリー・リストに ASAOLIB が要る。** 修飾名で CALL しても、プログラムが
...
try {
  ...
  osakaRef = sys.find((s) => s.name === "SR-OSAKA")?.ref;
  if (!osakaRef) throw new Error("SR-OSAKA が見つからない");
  await connect();
  ...
  for (const [pgm, dspf] of PROGRAMS) {
    ...
    try {
      ...
      const html = await call("get_screen_history_html", {
        ...
        note: `SR-OSAKA${HOST ? `(${HOST})` : ""} / CCSID ${CCSID_OVERRIDE ?? 5026} / CALL ${LIB}/${pgm} — 実行キーで送り、遷移した画面をすべて収録`,
        clear: true
      });
      const file = `${OUTDIR}/asaolib-${pgm}.html`;
