...
//
// 検証資材は scripts/build-gridtest3.mjs が作る ASAOLIB/GRIDTST5 ＋ GRIDCL7（背景→窓→OVERLAY）。
// CFxx を一切宣言していないので、どのファンクション・キーもホストがエラーで弾く。
//
//   AS400_HOST=... AS400_USER=... AS400_PASSWORD=... AS400_DEVNAME=AS01 node scripts/diag-window-fkey.mjs
...
const cmdField = session
  ...
  .find((f) => f.length > 20);
session.setField({ index: cmdField.index }, `CALL ASAOLIB/${PGM}`);
await session.sendAid("Enter", { timeoutMs: 20000 });
await sleep(1200);
dump(`CALL ASAOLIB/${PGM} 後`);
