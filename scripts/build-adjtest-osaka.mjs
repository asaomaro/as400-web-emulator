// SR-OSAKA の ASAOLIB に FFW の ADJUST（右寄せ）検証用の表示ファイル／RPG を作成・コンパイルする。
//
...
// そのビットを立てて送ってくるかを確かめる必要がある（→ research-adjust-osaka.mjs）。
//
// 実行: node --env-file=.env scripts/build-adjtest-osaka.mjs
import { readFileSync } from "node:fs";
import { Session5250 } from "@as400web/core";
import { SecretCrypto } from "../packages/server/dist/secret-crypto.js";

const LIB = "ASAOLIB", DDSF = "QDDSSRC", RPGF = "QRPGLESRC";
const log = (s) => process.stderr.write(s + "\n");
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
const rows = (s) => s.cells.map((r) => r.map((c) => c.char).join("").replace(/\s+$/, ""));

// ---- DDS 桁組み立て（build-feat-osaka.mjs と同じ規則）----
...
async function connectOnce(sys, password, dev) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // 入力プロンプトの見出しは共通。build-empsfl-osaka.mjs と同じ判定）
