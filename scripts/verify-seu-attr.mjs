// SR-OSAKA 実機で「SEU のソース行に埋め込まれた属性（制御コード）が、
// 制御コードに触れない編集をして保存しても失われない」ことを確かめる。
//
// 前提: ASAOLIB/QCLRTEST(ATTRTEST) の 1 行目に  C1C2C3 28 C4C5C6  が入っていること
...
const dev = process.env.AS400_DEVNAME ?? "AS01"; // 実機に定義済みの仮想装置（任意名の自動構成は効かない）
...
// --- STRSEU でメンバーを開く ---
session.setField({ index: cmd.index }, "STRSEU SRCFILE(ASAOLIB/QJPNTEST) SRCMBR(JPNATTR) OPTION(2)");
await enter(30000);
snap = session.snapshot();
