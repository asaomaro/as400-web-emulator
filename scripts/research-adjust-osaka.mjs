// SR-OSAKA の ASAOLIB/ADJPGM を呼び、**ホストが実際に送ってくる FFW** を生データストリームから
// 取り出して並べる（research 用）。`build-adjtest-osaka.mjs` で作った画面が前提。
//
...
//
// 実行: node --env-file=.env scripts/research-adjust-osaka.mjs
...
// ---------- 接続（build-adjtest-osaka.mjs と同方式） ----------
const captured = [];
