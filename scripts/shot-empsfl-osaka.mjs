// 実ブラウザ（web-ui）で SR-OSAKA へ接続し、EMPSFR のサブファイル画面を
// 先頭ページ／最終ページでスクリーンショットする。
// 前提: npm run build（web-ui dist）済み。実行: node --env-file=.env scripts/shot-empsfl-osaka.mjs
...
for (const s of cfg.sessions) if (s.name === "AS01") s.deviceName = dev;
...
try {
  ...
  log("app loaded");
  // システム SR-OSAKA を選択 → セッション AS01 に接続
  ...
  await inputs.nth(0).pressSequentially("ASAO", { delay: 50 });
  await sleep(150);
  await inputs.nth(1).pressSequentially("ASAO1234", { delay: 50 });
  await sleep(300);
  ...
  await cmd("ADDLIBLE ASAOLIB");
  await cmd("CALL ASAOLIB/EMPSFR");
  await sleep(800);
