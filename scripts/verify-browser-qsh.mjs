// 実ブラウザ（web-ui）で SR-OSAKA の **QSH（Qshell）** が使えるか検証する。
//
...
//
// 前提: npm run build＋web-ui の vite build 済み。`connections.json` に SR-OSAKA。
...
// **設定にある装置名（AS01）を先頭に置く**。実機に登録済みで、これが空いていれば一番早い。
// 続く WEBSF* は他のスクリプトと共用しているので、直前の検証が掴んでいると使えない
const DEV_POOL = (process.env.QSH_DEVNAMES ?? "AS01,WEBSF0,WEBSF1,WEBSF2,WEBSF3,WEBSF4").split(",");
