...
//
// 実行: node --env-file=.env scripts/verify-fresh-service-setup-osaka.mjs
//   （`.env` は**実機の接続先を渡すため**だけに使う。サーバーは別の空ディレクトリで
//    起動するので、master key も設定ファイルもそちらには無い）
//
// 副作用: 既存の仮想プリンター装置を借りる（既定 PRT_ASAO）。スプールは流さない。
...
try {
  ...
  await field(card, "名前").locator("input").fill("OSAKA");
  ...
  check(saved.systems?.[0]?.name === "OSAKA", "サーバー設定に入っている");
  check(
  ...
  await page.reload();
  await page.waitForSelector(".launcher", { timeout: 20_000 });
  const pick = page.locator(".card", { hasText: "OSAKA" }).first().locator("button", { hasText: "選択" });
  ...
  await page.reload();
  await page.waitForSelector(".launcher", { timeout: 20_000 });
  const pick2 = page.locator(".card", { hasText: "OSAKA" }).first().locator("button", { hasText: "選択" });
