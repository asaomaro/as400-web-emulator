// PR #231（IFS の上限表示・プレビュー競合・先回り判定）を SR-OSAKA で実測する。
//
// 確かめること:
...
// 画面操作の作法は `verify-browser-ifs-osaka.mjs` に合わせる（.card / .fn / .entries li / nav.crumbs）。
//
// 前提: npm run build && npm run build -w @as400web/web-ui。`connections.json` に SR-OSAKA。
// 実行: node --env-file=.env scripts/verify-ifs-limits-osaka.mjs
...
const sys = conn.systems.find((s) => s.name === "SR-OSAKA");
if (!sys) throw new Error("connections.json に SR-OSAKA がない");
...
try {
  ...
  const smallBody = "hello from SR-OSAKA\nにほんごの行\n";
  ...
  await page.locator(".card", { hasText: "SR-OSAKA" }).first().waitFor({ timeout: 30000 });
  await page.locator(".card", { hasText: "SR-OSAKA" }).first().locator("button", { hasText: "選択" }).click();
  ...
  // /home/ASAO/test まで辿る。
  // **一覧に出る名前は `ASAO`（大文字）。** IFS は解決時に大小を区別しないので
  ...
  check(
    ...
    raceText.includes("/test/small.txt") && raceBody.includes("hello from SR-OSAKA"),
    `見出し=${raceText.slice(0, 60)} / 本文=${raceBody.replace(/\s+/g, " ").slice(0, 60)}`
  );

  // ---- L4b: zip の文言 -------------------------------------------------------
  // 1 つ上へ戻り、test フォルダを選んでから「まとめてダウンロード」
  await page.locator("nav.crumbs button", { hasText: /^ASAO$/i }).first().click();
