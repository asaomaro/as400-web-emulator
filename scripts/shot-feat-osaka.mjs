// 実ブラウザ(web-ui)で SR-OSAKA へ接続し、FEATPGM の 3 画面を SO/SI 表示 ON で撮る。
// 前提: npm run build 済み。実行: node --env-file=.env scripts/shot-feat-osaka.mjs
...
try {
  ...
  await type("CALL ASAOLIB/FEATPGM"); await clickEnter(); await sleep(1500);
