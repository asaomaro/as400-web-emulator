// 実ブラウザ(web-ui, enhanced=true)で SR-OSAKA へ接続し、EXTPGM の拡張5250 GUI を撮る。
// 実行: node --env-file=.env scripts/shot-ext-osaka.mjs
...
// AS01 セッションに enhanced=true を付けて拡張5250 を広告する
const cfg = JSON.parse(readFileSync("connections.json", "utf8"));
for (const s of cfg.sessions) if (s.name === "AS01") s.enhanced = true;
...
try {
  ...
  await page.waitForSelector(".card:has-text('AS01')", { timeout: 10000 });
  await page.click(".card:has-text('AS01') >> button:has-text('接続')");
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if ((await has("サイン") && await has("オン")) || await has("パスワード")) {
      ...
      if (await inputs.count() >= 2) {
        await inputs.nth(0).pressSequentially("ASAO", { delay: 45 });
        await sleep(120);
        await inputs.nth(1).pressSequentially("ASAO1234", { delay: 45 });
        await sleep(150);
        await clickEnter();
  ...
  await type("ADDLIBLE ASAOLIB"); await clickEnter(); await sleep(1200);
  await type("CALL ASAOLIB/EXTPGM"); await clickEnter(); await sleep(1600);
