...
//
// 実行: node --env-file=.env scripts/verify-pane-state-osaka.mjs
...
writeFileSync(
  ...
  JSON.stringify({
    systems: [
      { id: "OSAKA", name: "OSAKA", host, ccsid: 5035, signon: { user, passwordEnv: "AS400_PASSWORD" } },
      // 切替の相手。**繋がない**（タブを開かないので接続は起きない）
      { id: "OTHER", name: "OTHER", host, ccsid: 5035, signon: { user, passwordEnv: "AS400_PASSWORD" } }
    ],
    sessions: [{ id: "DSP", name: "DSP", system: "OSAKA", sessionType: "display" }]
  })
);
...
try {
  ...
  await goto("システム");
  await page.locator(".card", { hasText: "OSAKA" }).first().locator("button", { hasText: "選択" }).click();
  await sleep(900);
  check((await typedValue()) === TYPED, "戻っても残る");
