...
//         scripts/research-device-busy-osaka.mjs
//
// 副作用: **自分の設定にある装置名**（既定 AS01）で 2 本開くだけ。装置は作らない・消さない。
 ...
 * （実際 `AS01` は使用中だった）、(b) 未使用の名前だとホストが**装置を自動作成**しうる。
 * 共用の本番機なので、どちらも避ける。
 *
...
let second;
try {
  const DEV = process.env.AS400_DEVNAME ?? "AS01";
  log(`1 本目を開く（装置名 ${DEV}）`);
