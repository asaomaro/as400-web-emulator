// 修正後の connections.json で SR-OSAKA へ再接続し、ASAOLIB の中身を一覧する。
// 実行: node --env-file=.env scripts/list-asaolib.mjs
...
const client = new Client({ name: "list-asaolib", version: "0" });
...
try {
  ...
  const osaka = sys.find((s) => s.name === "SR-OSAKA");
  if (!osaka) throw new Error("SR-OSAKA が見つからない");

  // system 参照で開く（装置名はホスト採番。保存済みの資格情報で自動サインオン）
  sessionId = (await call("open_session", { system: osaka.ref })).structuredContent.sessionId;
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (isSignon(s)) {
      ...
      await call("signon", { sessionId, system: osaka.ref });
    } else {
  ...
  await runCmd("DSPLIB LIB(ASAOLIB) OUTPUT(*)");
  const pages = [];
  for (let i = 0; i < 12; i++) {
    const s = await snap();
    pages.push(s.text);
