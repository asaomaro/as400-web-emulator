// research: 拡張5250（ASAOLIB/EXTPGM）の gui 実データと、F キー凡例検出の共存を実機で確認する。
...
try {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (await has("サイン") && (await page.locator("input.grid-input").count()) >= 2) {
      ...
      await inp.nth(0).pressSequentially("ASAO", { delay: 40 });
      await inp.nth(1).click(); await page.keyboard.press("Home");
      await inp.nth(1).pressSequentially("ASAO1234", { delay: 40 });
  ...
  await type("ADDLIBLE ASAOLIB"); await clickEnter(); await sleep(1300);
  await type("CALL ASAOLIB/FEATPGM"); await clickEnter(); await sleep(1700);
