// SR-OSAKA で実際にスプールを1件作り、MCP の host_get_spool(format=html) で HTML に落とす。
// pull 型（ホストサーバー経由）なのでプリンターセッションもライターも要らない。
//
// 実行: node --env-file=.env scripts/shot-spool-html-osaka.mjs <出力先.html>
...
try {
  ...
  const source = { system: sys.find((s) => s.name === "SR-OSAKA").ref };

  // 1. 帳票を1件作る（DBCS を含む一覧にしたいので DSPLIB を使う）
  const cmd = json(await call("host_command", { ...source, command: "DSPLIB LIB(ASAOLIB) OUTPUT(*PRINT)" }));
  log(`DSPLIB: ok=${cmd.ok ?? cmd.success} ${JSON.stringify(cmd.messages ?? []).slice(0, 120)}`);

  // 2. 自分のスプールを新しい順に探す
  const list = json(await call("host_list_spools", { ...source, filter: { user: "ASAO" }, max: 20 }));
  ...
  const html = json(await call("host_get_spool", {
    ...
    title: `SR-OSAKA スプール ${target.fileName}`,
    note: `DSPLIB LIB(ASAOLIB) OUTPUT(*PRINT) / CCSID 5026 — MCP host_get_spool(format=html)`
  }));
