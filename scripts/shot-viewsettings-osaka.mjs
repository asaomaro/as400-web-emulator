// 実ブラウザ(web-ui)で SR-OSAKA へ接続し、画面設定の「配色(端末色/意味色)」と
// 「画面の質感(CRT/フラット)」の before/after を撮る。
// 前提: npm run build 済み。実行: node --env-file=.env scripts/shot-viewsettings-osaka.mjs
...
try {
  ...
  // 装置 AS01 は前ジョブが掴んでいると "socket closed / 使用中" で落ちる。数十秒で解放されるのでリトライ。
  ...
  if (!connected) throw new Error("could not connect (device AS01 busy)");
  await sleep(900);
