// PC コマンド（STRPCO / STRPCCMD）の実機検証（SR-OSAKA）。
//
// ホスト側のテスト CL `ASAOLIB/PCOTEST`（`scripts/build-pcotest-osaka.mjs` で作成）が
...
//
// 実行: node --env-file=.env scripts/verify-pcocmd-osaka.mjs
import { readFileSync, existsSync, rmSync } from "node:fs";
import { Session5250, CommandConnection } from "@as400web/core";
...
if (!sys) { log("connections.json に SR-OSAKA がない"); process.exit(1); }
