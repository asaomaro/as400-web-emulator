// 実ブラウザ（web-ui）で SR-OSAKA の IFS ペインを一通り操作する E2E。
//
// `/home/ASAO/TEST` を作り、**画面の操作だけ**で次を確かめる:
...
// 実行: node --env-file=.env scripts/verify-browser-ifs-osaka.mjs
...
const DIR = `${BASE}/TEST`;
const TMP = process.env.IFS_TMP ?? "/tmp/as400-verify-ifs-osaka";
...
try {
  ...
  check("/home/ASAO まで辿れる", crumbs.includes("home") && crumbs.includes("ASAO"), crumbs);
  await shot("01-asao");
  ...
  await page.locator("nav.crumbs button", { hasText: "ASAO" }).first().click();
  ...
  if (left.includes("TEST")) {
    await api("delete", { path: DIR, recursive: true });
    log("後始末: /home/ASAO/TEST を API で削除した");
  }
