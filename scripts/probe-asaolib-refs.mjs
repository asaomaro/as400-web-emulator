// ASAOLIB の各プログラムがどの表示装置ファイル(DSPF)を使うかを DSPPGMREF で確かめる。
// 名前の規約（XXXPGM ↔ XXXDSPF）に頼らず、実機の参照情報で決める。
// 実行: node --env-file=.env scripts/probe-asaolib-refs.mjs
...
try {
  ...
  for (let i = 0; i < 6; i++) {
    const s = await snap();
    if (cmdField(s)) break;
    if (isSignon(s)) await call("signon", { sessionId, system: osaka.ref });
  ...
  await runCmd("DSPPGMREF PGM(ASAOLIB/*ALL) OUTPUT(*)");
  const pages = await pageAll();
  log("========== DSPPGMREF ASAOLIB/*ALL ==========");
