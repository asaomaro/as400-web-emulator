// SR-OSAKA の ASAOLIB に **FFW の挙動ビット**検証用の表示ファイル／RPG を作成・コンパイルする。
//
...
//
// 前作 `build-adjtest-osaka.mjs` と同じ組み立て規則・同じ実行系。
//
// 実行: node --env-file=.env scripts/build-ffwtest-osaka.mjs
...
// ---- DDS 桁組み立て（build-adjtest-osaka.mjs と同じ規則）----
...
// ============ 実行系（build-adjtest-osaka.mjs と同方式）============
...
try {
  ...
  if (!ok) failed++;
  log("\n【後続への申し送り】research-ffw-osaka.mjs の labels は上の OK 順（" +
