...
// **装置名を固定したまま比較を回さない。** AS01 を使い回すと、前回の切断が残っている限り
// 「対話式ジョブの回復」画面から始まり、比較したい画面に辿り着けない。
// 設定を写して装置名だけ空にし、ホスト採番に任せる（shot-ext-osaka.mjs と同じ流儀）。
const cfg = JSON.parse(readFileSync("connections.json", "utf8"));
for (const sess of cfg.sessions) if (sess.name === "AS01") sess.deviceName = "";
...
try {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    if (t.includes("対話式ジョブの回復")) { await type("90"); await clickEnter(); }
      ...
      await inputs.nth(0).pressSequentially(process.env.AS400_USER ?? "ASAO", { delay: 40 });
      await sleep(120);
