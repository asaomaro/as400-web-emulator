// 調査ラウンド 3: テスト用 CL（ASAOLIB/PCOTEST・PCOLONG）を実行し、
//   (1) 200 文字コマンドがデータストリーム上でどう並ぶか
//   (2) エミュレーターが実行キーを返したあと CL が STRPCCMD の**先へ進む**か（データ域 PCOMARK）
// を実測する。実行: node --env-file=.env scripts/research-strpco-osaka3.mjs [出力先]
...
try {
  ...
  for (const pgm of ["PCO123"]) {
    label = pgm;
    await run(`CALL ASAOLIB/${pgm}`, 30000);
  ...
  await run("CALL ASAOLIB/PCOTEST", 30000);
