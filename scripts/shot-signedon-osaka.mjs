// SR-OSAKA にサインオンし、認証後の画面を MCP 経由で HTML に落とす。
//
// 資格情報はこのファイルに書かない。実行時に環境変数で渡す:
//   AS400_USER=... AS400_PASSWORD=... node --env-file=.env scripts/shot-signedon-osaka.mjs out.html
//
...
const OUT = process.argv[2] ?? "signedon-sr-osaka.html";
const HOST = process.env.AS400_HOST;
const USER = process.env.AS400_USER;
const PASSWORD = process.env.AS400_PASSWORD;
// SR-OSAKA の SBCS はカタカナ側（290）。939 で繋ぐと F キー行のカタカナが英小文字に化ける
// （scripts/probe-ccsid-osaka.mjs で実測）
...
try {
  ...
  await shot(`${OUT}-01-signon-info.html`, "SR-OSAKA 認証直後（サインオン情報）", meta);
  ...
  log(text);
  log("----------------------------");
  await shot(`${OUT}-02-menu.html`, "SR-OSAKA 認証後の画面（メニュー）", meta);
