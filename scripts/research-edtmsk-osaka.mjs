// SR-OSAKA で **EDTMSK つき入力欄がワイヤ上どう来るか**を実測する。
//
...
// 独立にパースして SF オーダーを並べる（`research-adjust-osaka.mjs` と同じ作法）。
//
...
async function connectOnce(sys, password, dev) {
  const s = await Session5250.connect({
    ...
    // **hex は warn 経由でしか取れない**（`session.ts:400`）。`research-adjust-osaka.mjs` と同じ作法
    warn: (w) => {
