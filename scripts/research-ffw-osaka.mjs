// SR-OSAKA で **FFW の挙動ビット**を実測する（research 用）。前作 `research-adjust-osaka.mjs` の
// 生データストリーム捕捉をそのまま使い、対象を ADJUST から挙動ビット全体へ広げる。
//
// 【実験 A】ホストは CHECK(ME) / CHECK(MF) を**自分で検証するのか**
//   既存の `ASAOLIB/ADJPGM`（前作で作成済み）を呼び、CHECK(ME) 欄を**空**・CHECK(MF) 欄を
...
//   `build-ffwtest-osaka.mjs` で作った `ASAOLIB/FFWPGM` を呼び、SF オーダーの FFW を並べる。
//
// FFW の解釈は core を通さず**この場で独立に**パースする（検証対象の実装に依存させない）。
//
// 実行: node --env-file=.env scripts/research-ffw-osaka.mjs
...
// ---------- 生レコードから SF オーダーの FFW を拾う（research-adjust-osaka.mjs と同一）----------
...
const sys = conns.systems.find((s) => s.name === "SR-OSAKA");
...
try {
  ...
  if (inputs.length >= 9) {
    ...
    log("ADJPGM の画面が期待と違う。前作の build-adjtest-osaka.mjs を先に流すこと");
  ...
  // build-ffwtest-osaka.mjs の**単独コンパイルで通った順**。`Y`（num-only）は小数位が必須で
