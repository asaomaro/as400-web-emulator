...
describe("CL への差し込みを塞ぐ", () => {
  ...
  it("**本文の引用符は二重化する**（早じまいさせない）", () => {
    const cmd = buildSendCommand({ source: src, text: "it's ok", toUser: "ASAO" });
  ...
  it("**改行・制御文字は拒否**（コマンドが途中で切れる）", () => {
    for (const bad of ["a\nb", "a\rb", "a\x00b", "a\x1bb"]) {
      expect(() => buildSendCommand({ source: src, text: bad, toUser: "ASAO" })).toThrowError();
    }
  });
...
describe("組み立て", () => {
  ...
  it("小文字は大文字にそろえる（IBM i の名前は大文字）", () => {
    expect(buildSendCommand({ source: src, text: "x", toUser: "asao" })).toContain("TOUSR(ASAO)");
  });
});
