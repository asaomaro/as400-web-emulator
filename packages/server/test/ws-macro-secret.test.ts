...
describe("ws: マクロの秘密の差し込み", () => {
  it("所有者なら復号した平文がホストへ書かれる", async () => {
    ...
    expect(setField).toHaveBeenNthCalledWith(1, { index: 1 }, "MARO");
  ...
  it("他人のマクロを指す参照は拒否され、**1 欄も書かれない**", async () => {
    ...
    // 先に解決してから書くので、平文でない欄（MARO）も書かれない＝中途半端な状態を残さない
  ...
  it("秘密を含まない従来どおりの fields は影響を受けない（回帰）", async () => {
    const { conn, sent, setField } = await setup({ user: alice });
    await conn.handle(
      JSON.stringify({ type: "key", key: "Enter", fields: [{ field: 1, value: "MARO" }] })
    );
    expect(sent.at(-1)?.type).toBe("key-done");
    expect(setField).toHaveBeenCalledWith({ index: 1 }, "MARO");
  });
