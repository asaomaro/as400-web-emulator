...
describe("受信して積んで配る", () => {
  it("届いたエントリが履歴に入り、購読者へ配られる", async () => {
    ...
    const w = await reg.start({ ref: "own:c1", label: "MARO1/ORDERQ", spec: SPEC, connect: CONNECT });
    await settle();
...
describe("停止と障害を区別する", () => {
  ...
  it("**待っても直らない失敗は再試行せず error にする**（権限が無い）", async () => {
    ...
    await reg.start({ ref: "own:c1", label: "l", spec: SPEC, connect: CONNECT });
    await settle();
    a.failWith(new As400Error("ACCESS_DENIED", "not authorized to MARO1/ORDERQ"));
    await settle();
...
describe("待ち方", () => {
  it("**wait は常に無限**（HTTP の「無限待ち禁止」はここには効かない）", async () => {
    ...
    await reg.start({ ref: "own:c1", label: "l", spec: SPEC, connect: CONNECT });
    await settle();
    expect(spy.mock.calls[0]?.[0]).toMatchObject({ name: "ORDERQ", library: "MARO1", wait: -1 });
    reg.closeAll();
  });
  ...
  it("キー付きキューはキーと検索条件を渡す", async () => {
    ...
    await reg.start({
      ref: "own:c1",
      label: "l",
      spec: { library: "MARO1", name: "KEYQ", key: "AB", search: "GE" },
      connect: CONNECT
    });
    await settle();
