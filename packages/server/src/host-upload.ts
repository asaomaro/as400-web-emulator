 ...
 *
 * ⚠ **このルートは IBM i に書き込む。** 以前は「ホスト API のうち書き込むのはここだけ」
 * だったが、2026-07-30 に `/api/host/sql` も非クエリ文（DML / DDL）を実行するようになった
 * （`host-sql.ts` の冒頭。認可の考え方はこちらと同じ結論を踏襲している。design DD6）。
 *
