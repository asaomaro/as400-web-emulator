...
export class IfsConnection {
   ...
   * 置き換えでも実機は元のタグを保つ——`verify-browser-ifs-osaka.mjs` と
   * `research-ifs-dataccsid-osaka.mjs` の両方で確認）。
   *
   * **指定しないと中身と食い違うタグが付く。** 既定の 0 は「サーバー既定」で、
   * 中身に関係なく実機の既定が付く（SR-OSAKA は **1041** / PUB400 は **850**）。
   * 読む側は決定表①（中身推定）で救えるが、**他ツールから見ると嘘のタグ**になるので、
   * **どの文字コードで符号化したか分かっている経路は必ず渡すこと**
   * （`/api/host/ifs/write` のテキスト保存は渡している。`20260801-ifs-write-dataccsid`）。
   * バイナリ（base64・アップロード）は文字コードの概念が無いので渡さない。
   */
  async writeFile(
    path: string,
    data: Uint8Array,
