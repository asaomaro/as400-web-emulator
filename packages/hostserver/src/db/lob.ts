 ...
 * （実機 SR-OSAKA で確認。`20260802-lob-multi-segment` の research F1〜F3）。
...
export async function retrieveLob(
  ...
  for (;;) {
    ...
    // 3 バイトに切られる（実機 SR-OSAKA で踏んだ。`20260801-dbclob-locator-decode`）。
 ...
 * `-401` は原典のコメントが挙げる値、`-816` は**実機 SR-OSAKA（7.3）が実際に返した値**。
 * どちらも二重解放の合図なので騒がない。
 */
...
export async function freeLob(conn: DbConnection, locator: number): Promise<boolean> {
  try {
    ...
    if (t.rcClass !== 0) {
      ...
      // **実機 SR-OSAKA（IBM i 7.3）は `2 / -816` を返した**——原典自身が
