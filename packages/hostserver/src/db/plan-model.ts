...
export interface MonitorRecord {
   ...
   * 2 表で 1・2、3 表で 1・2・3 になるのを SR-OSAKA 7.3 で実測した。
   ...
   * 意味のある違いはこの 1 列だけだった（`N` → `Y`）。SR-OSAKA 7.3 実測。
   */
  QVC14: string | null;
 ...
 * 使った索引。
 *
 * **`3001` は `QQ1000` に実際のアクセスパス名を持つ**（`Q_ASAOLIB_M_MENU_MENUCD_00001` のような
 ...
 * SR-OSAKA 7.3 で確認できたのは `NL` だけ。それ以外はコードのまま見せて、
