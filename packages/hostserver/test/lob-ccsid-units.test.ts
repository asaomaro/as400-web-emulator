 ...
 *
 * 実機 SR-OSAKA（IBM i 7.3）で両方を確かめた
 * （`scripts/research-dbclob-locator-osaka.mjs` / `20260801-dbclob-locator-decode`）:
 *
 * | 列 | CCSID | 値 | 申告長 | 実際の本体 |
 * |---|---|---|---|---|
 ...
 * **実機 SR-OSAKA の BLOB は `65535` で来る**——`0` ではない
 * （`scripts/research-lob-big-dbcs-blob-osaka.mjs` F3 で実測）。
 *
