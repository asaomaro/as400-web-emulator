 ...
 * 実行計画の畳み込み。
 *
 * 固定値は **`20260802-sql-visual-explain` の design 工程で SR-OSAKA(7.3) から実測したもの**
 ...
 * SR-OSAKA 7.3 で実測した（`design.md` A1 の訂正）。
 */
describe("結合の木", () => {
  ...
  it("2 つのダイヤルを左深に組む（ACS と同じ読み方になる）", () => {
    const plan = buildQueryPlan(
      [
        joined({ QQRID: 3000, QQJNP: 1, QVQLIB: "ASAOLIB", QVQTBL: "M_MENUTR" }),
        joined({ QQRID: 3001, QQJNP: 2, QVQLIB: "ASAOLIB", QVQTBL: "M_MENU" })
      ],
      META
    );
...
describe("導いた節（テーブル・プローブ／最終選択）", () => {
  ...
  it("索引だけで足りないダイヤルの後ろにテーブル・プローブを挟む（`QVC14='N'`）", () => {
    const plan = buildQueryPlan(
      [
        dial({ QQRID: 3000, QQJNP: 1, QVQLIB: "ASAOLIB", QVQTBL: "M_MENUTR" }),
        dial({ QQRID: 3001, QQJNP: 2, QVQLIB: "ASAOLIB", QVQTBL: "M_MENU", QVC14: "N" })
      ],
      META
    );
    const root = plan.blocks[0]?.joinTree;
  ...
  it("**索引だけで足りるなら挟まない**（`QVC14='Y'`）", () => {
    const plan = buildQueryPlan(
      [
        ...
        dial({ QQRID: 3001, QQJNP: 2, QVQLIB: "ASAOLIB", QVQTBL: "M_MENU", QVC14: "Y" })
      ],
      META
    );
  ...
  it("索引名は `QQ1000` のアクセスパス名を使う（ACS と同じ名前になる）", () => {
    const plan = buildQueryPlan(
      [rec({ QQRID: 3001, QVQLIB: "ASAOLIB", QVQTBL: "M_MENU", QVINAM: "M_MENU", QVILIB: "ASAOLIB", QQILNM: "ASAOLIB", QQ1000: "Q_ASAOLIB_M_MENU_MENUCD_00001" })],
      META
    );
    const node = plan.blocks[0]?.nodes[0];
    expect(node?.index?.name).toBe("Q_ASAOLIB_M_MENU_MENUCD_00001");
    expect(node?.label).toBe("索引の使用: Q_ASAOLIB_M_MENU_MENUCD_00001");
  });
