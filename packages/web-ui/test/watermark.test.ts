...
describe("差し込み変数の展開", () => {
  it("既知の変数は値に置き換わる", () => {
    expect(expandWatermarkText("{system} / {user}", { system: "本番機", user: "MARO" })).toBe(
      "本番機 / MARO"
    );
  });
...
describe("エミュレーター画面への適用", () => {
  it("セッション設定に透かしがあれば重ねる（変数は接続の実値で展開）", async () => {
    openPane({ text: "{system} {device} {user}" }, { name: "WEBEMU01", user: "MARO" });
    const w = mount(EmulatorPane, { props: { sessionId: SID, focused: true }, attachTo: document.body });
    await nextTick();
    expect(w.find(".wm-line").text()).toContain("本番機 WEBEMU01 MARO");
    w.unmount();
  });
