...
const W2 = { ...W1, id: "w2", ref: "own:c2", label: "MARO1/LOGQ" };
...
describe("watchesStore: サーバーの写し", () => {
  ...
  it("watch-list で一覧が入り、先頭が選ばれる", () => {
    ...
    expect(watchesStore.watches.map((w) => w.label)).toEqual(["MARO1/ORDERQ", "MARO1/LOGQ"]);
...
describe("WatchPane", () => {
  ...
  it("一覧に受信・未読・状態が出る", async () => {
    ...
    expect(row.text()).toContain("MARO1/ORDERQ");
