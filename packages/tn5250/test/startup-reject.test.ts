 ...
 *
 * ⚠ 実機 SR-OSAKA では**装置名の重複でこの経路に入らない**——ホストは理由を返さず
 * ソケットを閉じる（`scripts/research-device-busy-osaka.mjs`）。よってレコードは合成する。
...
describe("表示セッションの起動応答", () => {
  it("**失敗コードは理由つきで断られる**（装置名が入っていなくても取りこぼさない）", async () => {
    const r = await connectWith(startupRecord("8902"), "AS01");
  ...
  it("**要求した装置名を文言に添える**（失敗応答には入っていないので利用者が直せない）", async () => {
    ...
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.message).toContain("AS01");
  });
  ...
  it("成功コードは従来どおり（起動応答として控えて先へ）", async () => {
    const r = await connectWith(startupRecord("I902", "S7857290", "AS01"), "AS01");
 ...
 * ——**プロセスが落ちる**。実機 SR-OSAKA で装置名が使用中のときに踏んだ。
 */
