...
export interface ParsedGridItem {
   ...
   *
   * SR-OSAKA で実測: `GRDLIN((*POS (4 3 40)) (*TYPE UPPER 3 2))` → 3, 2（3 本を 2 行おき）。
...
function parseWindow(r: ByteReader, decode: Decode): ParsedWindow {
  ...
  while (r.remaining >= 2) {
    ...
    if (borderType === 0x10 && body.length > 4) {
      ...
      //
      // **罫線文字は「あれば」**。実機（SR-OSAKA）で `WDWBORDER((*COLOR PNK))` を出すと
...
function parseGridLines(r: ByteReader): ParsedGridLines {
  ...
  // 実機（SR-OSAKA）は「バッファをクリアせよ」だけのとき `01 80 00 00 00` ＝
  ...
  // 実機（SR-OSAKA）は `length=0x0b`＝**11 バイト**で送ってきた。増えているのは
