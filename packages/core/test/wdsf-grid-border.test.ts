...
describe("CREATE WINDOW の Border Presentation（WDWBORDER）", () => {
  ...
  /**
   * **実機（SR-OSAKA）が実際に送ってきた形**。`WDWBORDER((*COLOR PNK))` は色だけの指定なので、
...
/**
 * **実機（SR-OSAKA）が送ってきたグリッド線のバイト列そのもの。**
 *
...
/**
 * **色・線種・種別の対応を実機で 1 つずつ確かめる**（ASAOLIB/GRIDTST3・GRIDCL3）。
 *
