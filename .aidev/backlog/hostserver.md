...
- [ ] **LOB フィールドしきい値を上げて往復を消す道を検討する**（上の調査の続き）
  - 原典が往復を減らすのは**まとめ取りではなくしきい値**（`0x3822`）。
    超えた LOB だけがロケーターになり、以下は**行データに載って返る**（追加の往復 0）。
    JTOpen 既定 **32,768 バイト**／上限 **15,728,640**（公表 16,777,216 だが通らない。
    「ODBC に合わせる」）。単位は**バイト**——`JDMRI.java` の「in kilobytes」は**誤り**
  - こちらは `LOB_FIELD_THRESHOLD = 0`（`db-connection.ts:42`）。
    実測で DBCLOB(2M) の表を 2 行取っただけで応答が 8.4MB（0 なら 10KB）になったため
  - **実機が要る**: しきい値以下の LOB がインラインで来たとき、`db-decode.ts:110-120` の
    「型コードで判定して 4 バイトのロケーターを読む」が成立するかは実測しないと分からない
    （外すと以降の列が全部ずれる）。CLOB でしか試していない点も未解消
...
- [ ] ロケーターの明示的な解放を**実装する**（接続を閉じれば消えると見込んでいるが未確認）
  - 要求形式は上のとおり確定した。残るのは**実装と実機確認**——
    単発接続では閉じれば消えるはずなので、**入れる価値があるかの判断**も含む
  - `20260720-sql-lob-locator/research.md` F5 では、ロケーターが**接続に紐づく**こと
    （別接続では `rc=2/-815`）までは実測している
