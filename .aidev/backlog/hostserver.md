...
- [ ] **常駐プリンターを画面から扱えるようにする**（上の続き）
  - サーバー側は入ったが **UI が無い**。`GET /api/printers` は口を作っただけで、
    「開き直したら常駐中の一覧が見え、未読の帳票と出力の警告が読める」画面は未実装
  - **常駐はプロセス再起動で消える**（設定から起動し直す仕組みは無い）。
    「常駐」という言葉から永続を期待されうるので、UI で示すか別途決める
  - 長時間（数時間〜数日）保つかは未測定。DTAQ 監視では 45 分のアイドルを越えられている
