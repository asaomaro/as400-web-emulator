...
## 判明した事実（すべて SR-OSAKA・IBM i 7.5 で実測）

再現手段: `scripts/research-sql-exec-osaka.mjs`
...
`CREATE TABLE ASAOLIB.SQLEXEC2 (ID INT)` は **`7905`（警告つき成功）**。
続けて同じ表をシステム命名（`ASAOLIB/SQLEXEC2`）で作ろうとすると `-601`（既に存在）＝
**どちらの命名でも解決される**。`DROP TABLE ASAOLIB.SQLEXEC2` も 0 で通った。
