...
（SR-OSAKA は `F4=ﾌﾟﾛﾝﾌﾟﾄ`）。語で判定すると言語ごとに壊れる。
...
| `scripts/build-dttest-osaka.mjs` / `research-edtmsk-osaka.mjs` / `research-sysval-osaka.mjs` | 新規（実測の再現手段） |
 ...
 * （SR-OSAKA は `F4=ﾌﾟﾛﾝﾌﾟﾄ`）。`key === "F4"` で判定し、**ラベルはホストのものを渡す**
 * （UI 側で言い換えない）。
 */
