...
SR-OSAKA（IBM i 7.5）に `ASAOLIB/DTMDSPF` + `DTMPGM` を作り
（`scripts/build-dttest-osaka.mjs`）、受信を実測した（`scripts/research-edtmsk-osaka.mjs`）。
...
> **独立パースの限界を正直に書く**: `research-edtmsk-osaka.mjs` の自作パーサは
...
`scripts/research-sysval-osaka.mjs` で実測:
...
- **ラベルは凡例から取る**。`F4` が必ず「プロンプト」とは限らず、SR-OSAKA では
