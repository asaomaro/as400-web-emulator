...
**実測**（ACS で `MARO1/TESTPF` を転送し、直後に接続先ポートを確認）:

```
netstat -n | findstr /i ":446 :8471"
TCP  192.168.0.95:53202  185.113.5.134:8471  ESTABLISHED
```

- **8471 = database サーバー**。DDM（446）への接続は**無い**。
- `ESTABLISHED` のまま残る＝ ACS は接続を張りっぱなしで使い回している。

ACS の接続検査が列挙するサービス一覧でも、**「データベース サービス（8471）」と
「レコード・レベル・アクセス サービス（446）」は別項目**である。
後者が DDM で、データ転送はそれを使っていない。

**この事実がひっくり返す前提**: 前作業 `20260719-hostserver-upload-ddm` の requirement は
DDM を選ぶ理由を「`query` は SELECT 専用で、`host_command` の `RUNSQL` は 1 文ずつしか
流せない」と書いていた。これは **IBM i 側の制約ではなく、我々の実装の制約**である。
database サーバーは prepare/execute で INSERT を実行できる。

→ backlog の項目名（「`jtopenlite/ddm/` 相当のレコードレベルアクセス」）が
そのまま土台の選定になり、**必要性が一度も検証されないまま**進んでいた。

### F12: DBCS 対応 CCSID の列を持つ表は DDM で開けない（T19 で判明）

| 表 | 列の CCSID | 作成方法 | `open` |
|---|---|---|---|
| `ZZUP1` | 273 のみ | SQL | ✅ 成功 |
| `ZZUP2` / `CSVUPJP` | **5035 / 930** | SQL | ❌ `CPF427D` ×2 |

作成方法が同じで CCSID だけが違うため、原因は CCSID で確定。
我々の UFCB は原典（jtopenlite `sendS38OpenRequest`）と**完全に一致**しており実装漏れではない。

あわせて DDS 作成の物理ファイル（`MARO1/TESTPF`）は
`CPF4135 Record format name for member TESTPF was not valid.` で開けない。
`open` の既定が「様式名＝ファイル名」だが、DDS ファイルでは一致しないことがある。

**F11 と合わせると**: これらは「DDM を使う限り付いて回る制約」であり、
本家が通っていない道を我々だけが通ろうとしていた。
