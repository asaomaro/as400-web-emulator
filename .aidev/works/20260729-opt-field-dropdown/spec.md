...
- 対象は**フォーカス中の欄が Opt 列に属するとき**だけ（一覧の全行に出すと画面が埋まる）
- 欄の直下にポップオーバーを開く。項目は `2 変更` のように**番号とラベル**を並べる
- 選ぶと欄に番号が入り、ポップオーバーを閉じてフォーカスを欄へ戻す
- 開く導線: 欄にフォーカスがある状態で**下矢印**、または欄の右端の小さな印をクリック
- `Esc` / 外側クリックで閉じる
