...
3. **`3000` は「表ごと」に出る**（結合で 2 件、対象表が `QVQTBL` に入る）。
   → 走査対象の一覧は `3000` から作れる。
4. **【2026-08-08 追記】`QQJNP` が結合の順位（ダイヤル）を持つ。** SR-OSAKA 7.3 で実測:
...
5. **【2026-08-08 追記】ACS が出している残り 2 つも記録から導ける。**
   ...
   | **索引名 `Q_ASAOLIB_M_MENU_MENUCD_00001`** | `3001` の `QQ1000`（＋ライブラリは `QQILNM`） | `QVINAM` は索引が載る**ファイル名**（`M_MENU`）で、ACS が出しているアクセスパス名ではない |
