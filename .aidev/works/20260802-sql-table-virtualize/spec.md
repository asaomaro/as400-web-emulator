...
| `scripts/research-sql-table-render-osaka.mjs`（作成済み） | 基準線 |
| `scripts/verify-sql-table-virtualize-osaka.mjs`（新規） | 前後の実測・幅が動かないこと |
...
- 実機（SR-OSAKA）は共用の本番機。**表を作らない**——`QSYS2.SYSCOLUMNS` から行を取る。
