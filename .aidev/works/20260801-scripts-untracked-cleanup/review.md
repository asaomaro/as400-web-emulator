...
- [should] `scripts/shot-asaolib-screens.mjs:21-25` — **env 化が既定の使い方を壊していた**。
...
- [should] `scripts/shot-asaolib-screens.mjs:180` — 上と対の問題。note の
  `SR-OSAKA(${HOST})` は**無条件**に HOST を埋める。保存値で開いた回は HOST が
  `undefined` になり、成果物の説明文が `SR-OSAKA(undefined)` になる。
  / 対応: **差し戻しに同梱**

- [nit] `scripts/shot-spool-html-osaka.mjs:35` — `filter: { user: "ASAO" }` がハードコード。
  資格情報ではなくスプールの絞り込み値で、`ASAOLIB` / `/home/ASAO` は追跡済みの
...
- **secret scan**: 29 本を再走査し、ハードコードされたパスワード・IP は **0 件**
  （`172.21.10.51` は `scripts/` から消えた）
- **README の族の本数**が実体と一致: `shot-*` 15 / `build-*-osaka` 3 / `probe-*` 3 /
  `check-*` 3 / `diff-*` 2 ＋単発 3 ＝ **29**（`git status` の実数で照合）
- **他 4 本の env 必須化は正しい**（`check-html-determinism` / `probe-ccsid-osaka` /
  `shot-signedon-osaka` / `shot-signon-osaka` はいずれも `open_session` で無条件に `host` を使う）
- **出力メタ文字列 2 か所**（`shot-signedon-osaka:87` / `shot-signon-osaka:54`）が
...
`shot-asaolib-screens.mjs` を、**HOST が本当に要るときだけ要求する**形に直した。

- `if (CCSID_OVERRIDE && !HOST)` に変更。既定（保存値で開く）は AS400_HOST 無しで動く。
  なぜ条件付きなのかを JSDoc で残した
- note を `SR-OSAKA${HOST ? \`(${HOST})\` : ""}` に変更。**分かっているときだけ書く**
  ——保存値で開いた回に固定文字列やダミーを出すと、どのホストを撮ったのか説明文が保証できない

### 再検証（実行結果）

- `node --check scripts/shot-asaolib-screens.mjs`: 通過
...
**なし**（must 0 / should 0 / nit 0）。ラウンド 1 の nit 1 件（`user: "ASAO"`）は据え置き。
