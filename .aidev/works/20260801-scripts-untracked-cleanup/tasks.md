...
- [x] T1: ホスト指定を `process.env.AS400_HOST` へ寄せる（既定値なし・未設定なら `exit(1)`）。
      `check-html-determinism.mjs:22` / `probe-ccsid-osaka.mjs:29` / `shot-asaolib-screens.mjs:21` /
      `shot-signedon-osaka.mjs:41,84` / `shot-signon-osaka.mjs:27,48`。
      **84 / 48 行は出力メタの文字列**——直し忘れると成果物に社内 IP が焼かれる
- [x] T2: 未使用変数 6 件を解消する。1 行のヘルパー 5 件（`has` × 4・`constant`）は**削除**、
      `shot-fkey-osaka.mjs:42` の `probe`（44 行）は **`_probe` に改名して残す**＋理由コメント（spec D2）
- [x] T3: `scripts/README.md` に 29 本の族の節を追記する（`shot-*` 15 / `build-*-osaka` 3 /
