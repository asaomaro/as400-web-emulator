...
`?? "172.21.10.51"` のような**既定値を持たせない**。既定があると
...
| `probe-ccsid-osaka.mjs:29` | 同上 |
| `shot-asaolib-screens.mjs:21` | `const HOST = …` |
| `shot-signedon-osaka.mjs:41` | `open_session` の `host` |
| `shot-signedon-osaka.mjs:84` | **出力メタの文字列**（画像に焼かれる説明文） |
| `shot-signon-osaka.mjs:27` | `open_session` の `host` |
| `shot-signon-osaka.mjs:48` | **出力メタの文字列** |
...
| 対象 | 規模 | 扱い | 理由 |
|---|---|---|---|
| `has` × 4（`shot-buttons-osaka` / `shot-crt` / `shot-empsfl-osaka` / `shot-font-osaka`） | 1 行 | **消す** | `(await page.locator("body").innerText()).includes(t)` の別名。要るときに 1 行で書き直せる |
| `constant`（`build-empsfl-osaka:40`） | 1 行 | **消す** | 同上（DDS 定数行の組み立て） |
| `probe`（`shot-fkey-osaka:42-84`） | **44 行** | **`_probe` に改名して残す** | 画面の DOM から GUI 要素・入力欄・凡例を読み `probe-<label>.json` に落とす探査関数。**書き直しは安くない**。診断スクリプトが必要時に呼ぶ道具を持つのは不自然でない |
...
| `build-*-osaka` | 3 | `ASAOLIB` に DDS/RPGLE のフィクスチャを作る |
| `probe-*` | 3 | 単発の実測（CCSID・窓の信号・`ASAOLIB` 参照） |
| `check-*` | 3 | 不変条件の確認（HTML の決定性・メニュー排他・永続化） |
| `diff-*` | 2 | 実機とこちらの出力の突き合わせ（罫線・web-ui vs ホスト） |
| `list-asaolib` / `research-ext-gui` / `verify-spool-html` | 各 1 | 一覧・調査・検証 |
...
**触らないもの**: 追跡済みの 73 本、`packages/`、既に公開済みの `172.21.10.51`
...
| `172.21.10.51` が `scripts/` に残らない | D1（7 か所）＋ `git grep` で確認 |
