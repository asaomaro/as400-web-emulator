...
実施 2026-08-01T09:19:52Z。**実機（SR-OSAKA）は使わない**——スクリプトは実機が要り、
...
| `172.21.10.51` が `scripts/` に 1 か所も残らない | **0 件**（`grep -rn … scripts/`） |
...
| `probe-ccsid-osaka.mjs` | 既存の `USER`/`PASSWORD` チェックに `HOST` を合流 |
| `shot-asaolib-screens.mjs` | `const HOST = "…"` → env ＋未設定チェック |
| `shot-signedon-osaka.mjs` | `HOST` 追加、`open_session`、**出力メタ文字列** |
| `shot-signon-osaka.mjs` | `HOST` 追加＋未設定チェック、`open_session`、**出力メタ文字列** |
...
- **削除 5 件**: `has`（`shot-buttons-osaka` / `shot-crt` / `shot-empsfl-osaka` / `shot-font-osaka`）・
  `constant`（`build-empsfl-osaka`）。いずれも 1 行で、要るときに書き直せる
- **`_probe` に改名 1 件**: `shot-fkey-osaka.mjs`（44 行の DOM 探査関数）。
  一度もコミットされておらず、消すと git 履歴にも残らない

## 未検証の穴（deliver へ引き継ぐ）

- **スクリプトを実行していない**。実機 SR-OSAKA が要る。したがって
...
- **既に追跡済みの `172.21.10.51`** は残る（`20260728-strpco-strpccmd/research.md` /
