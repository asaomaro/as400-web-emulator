...
+   * （実機 SR-OSAKA・YB0270R で確認。KSN20 罫線が「一度描かれた直後に消える」不具合の原因
...
+   * 実機（SR-OSAKA、PB1000R）のトレースで、CREATE WINDOW で出した窓を閉じて呼び出し元の
...
+   *
+   * 実機（SR-OSAKA、PB1000R）のトレースで、CREATE WINDOW の窓を閉じて呼び出し元へ戻るとき、
 ...
 *
 * 実機（SR-OSAKA）のトレースで、`WRITE KSN20`（罫線を描く）の直後に
