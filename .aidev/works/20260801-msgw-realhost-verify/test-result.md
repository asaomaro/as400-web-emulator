...
実施 2026-08-01。実機 **SR-OSAKA（IBM i 7.3・平文）**。
再現は `scripts/research-msgw-osaka.mjs`。

## 実機（8 項目すべて PASS）

```
プリンター起動: I902 device=PRT_ASAO
  QPRTLIBL#180 status=MESSAGE_WAIT form=AIDEVMSGW outq=PRT_ASAO
  PASS 検証対象のスプールが見つかる
  PASS 状態が MSGW（実際: MESSAGE_WAIT）

### retrieveMessage
  id=CPA3394
  text=(G B I H R C)装置PRT_ASAO書き出しプログラムPRT_ASAOの用紙タイプ'AIDEVMSGW'をロードしてください。
  handle=24 バイト
...
```
（修正前）text=(G B I H R C)çqãðPRT_ASAOå¸àgáºàýäPä]äBäÝäw...
（修正後）text=(G B I H R C)装置PRT_ASAO書き出しプログラムPRT_ASAOの用紙タイプ'AIDEVMSGW'をロード…
```
...
- 装置は `PRT_ASAO` を借りた。**他の装置・他人の OUTQ では試していない**
