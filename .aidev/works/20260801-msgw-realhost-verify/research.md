...
`PRT_ASAO` / `PRTASD` / `PRTNAGA` / `PASANOQ` / `PHAYAQ` / `PRT01` / `PRT21` / `SZKP1` ほか。

**ライターは 1 台も動いていない**（`QSYS2.OUTPUT_QUEUE_INFO` で `WRITER_JOB_NAME IS NOT NULL` が 0 行）。
利用者の指定により `PRT_ASAO` を借りた（**作らない・消さない**。ライターだけ起動して必ず止める）。

## F4. MSGW の作り方

1. `VRYCFG CFGOBJ(PRT_ASAO) CFGTYPE(*DEV) STATUS(*ON)`
2. `PrinterSession.connect(deviceName: "PRT_ASAO")` → `I902`（ホストがライターを起動する）
3. `CHGJOB OUTQ(PRT_ASAO)` でコマンドサーバーのジョブの出力待ち行列を移す
...
```
text=(G B I H R C)çqãðPRT_ASAOå¸àgáºàýäPä]äBäÝäwPRT_ASAO...
```

原因は `decodeNpString` が **CCSID 37 決め打ち**だったこと
（`netprint-connection.ts` の `const MESSAGE_CCSID = 37`）。
SR-OSAKA のサーバー CCSID は **5035** で、メッセージはジョブの文字コードで来る。
