...
## 実機 E2E の内訳（15 項目・SR-OSAKA / ASAOLIB）
...
- `npm run lint` が 6 件エラーを出すが、**すべて未追跡の既存スクリプト**
  （`shot-buttons-osaka.mjs` / `shot-crt.mjs` / `shot-empsfl-osaka.mjs` / `shot-fkey-osaka.mjs` /
  `shot-font-osaka.mjs` / `build-empsfl-osaka.mjs`）の未使用変数。前セッションの置き土産で、
