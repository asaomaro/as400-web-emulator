...
3. **実機 SR-OSAKA（ASAOLIB）で検証する**。ADJUST 指定欄を持つ DDS フィクスチャを作り、
...
- **ホスト**: SR-OSAKA。**ライブラリーは `ASAOLIB` のみ**（他ライブラリーを作成・変更しない）
- **資格情報**: 既存基盤に載せる。`connections.json` の `SR-OSAKA` の `signon.passwordEnc` を
...
- 実機 SR-OSAKA が ADJUST 指定欄をどう送ってくるか（FFW のビットが実際に立つか）
