...
Java 実装）と、**SR-OSAKA の実測**の 2 本立てで行う（AGENTS.md「既存プロトコル実装の移植」）。
...
## 2. 実機（SR-OSAKA / ASAOLIB）

### 2.1 フィクスチャ

`scripts/build-adjtest-osaka.mjs` で `ASAOLIB/ADJDSPF` + `ASAOLIB/ADJPGM` を作成（冪等）。
...
（`ライブラリー ASAOLIB にファイル ADJDSPF が作成された。`）。数値欄限定ではない。

### 2.2 FFW の実測

`scripts/research-adjust-osaka.mjs` が生データストリームを捕まえ、**core を通さず独立に**
...
`scripts/research-adjust-roundtrip-osaka.mjs` で、同じ値を **(A) 左詰めのまま**（＝右寄せ未実装の
