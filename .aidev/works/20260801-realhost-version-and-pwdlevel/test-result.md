...
実施 2026-08-01。実機 **SR-OSAKA**（社内 LAN・平文接続）。
...
```
host=172.21.10.51 tls=false user=ASAO
認証成功
...
```
host=172.21.10.51 tls=false V7R3M0 ccsid=5035 dsLevel=11
...
→ **VRM と PTF の 2 経路が一致。SR-OSAKA は IBM i 7.3。**
...
| 訂正対象の面に誤った版数表記が残っていないこと | **0 件**（`grep -rn "SR-OSAKA.*IBM i 7\.5" packages/*/src packages/*/test`） |
...
- **TLS は確認していない**。SR-OSAKA へは平文で接続した。
  レベル 0 × TLS の組み合わせは未実測
- **DDM(DRDA) は試していない**。SECCHK が SHA 前提のため、レベル 0 の SR-OSAKA では
