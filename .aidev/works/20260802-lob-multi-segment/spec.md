...
| `scripts/research-lob-multi-segment-osaka.mjs`（作成済み） | 事実の採取 |
| `scripts/verify-lob-multi-segment-osaka.mjs`（新規） | 直ったことの実機確認 |
...
- 実機（SR-OSAKA）は共用の本番機。表は自分のライブラリーに作り `finally` で消す。
