...
**原典の直読**（GNU tn5250 / tn5250j）と **SR-OSAKA の実測**の 2 本立て。
...
## 1. 実機の実測（SR-OSAKA / IBM i 7.5 / CCSID 939）

`scripts/build-sgntest-osaka.mjs` で `ASAOLIB/SGNDSPF`・`SGNPGM` を作り、
`scripts/research-sign-osaka.mjs` が**欄ごとに 1 つずつ**送って RPG の `[...]` を読んだ。
