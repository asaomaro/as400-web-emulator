...
## 実機検証（SR-OSAKA / IBM i 7.3）

### 事実の採取

`scripts/research-lob-big-dbcs-blob-osaka.mjs`（`research.md` に全文）。
...
`scripts/verify-lob-big-dbcs-blob-osaka.mjs` — **18 passed / 0 failed**。

```
フィクスチャ: ASAOLIB.LOBBIGV
...
後片付け: `finally` で `DROP TABLE`。表は自分のライブラリー（`AS400_LIB`、既定 `ASAOLIB`）。
内部 IP はスクリプトに書いていない。
