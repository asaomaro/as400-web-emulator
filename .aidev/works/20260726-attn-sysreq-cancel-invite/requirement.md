...
正体を実機（SR-OSAKA / system `S7857290`）で確かめたところ、この入力欄は **Attn（アテンション）キー**の結果だった。
...
- **コマンド入力窓そのものの描画**。あれはホストの ATNPGM が描くもので、端末の責務ではない。
  ATNPGM の内容はシステム設定（`EVXX01` は SR-OSAKA 固有）なので、こちらで模倣しない。
...
- [ ] **実機（SR-OSAKA）で Attn を押すとコマンド入力窓が出て、コマンドを実行でき、F3 で背面が戻る**。
