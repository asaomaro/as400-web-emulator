...
**SR-OSAKA の実測**の 2 本立て（AGENTS.md「既存プロトコル実装の移植」／前作 #205 と同じやり方）。
...
## 2. 実機（SR-OSAKA / IBM i 7.5 / CCSID 939）

### 2.1 実験 A: **ホストは CHECK(ME) / CHECK(MF) を検証しない**（U3 の答え）

前作の `ASAOLIB/ADJPGM` をそのまま使い、**CHECK(ME) 欄を空・CHECK(MF) 欄を部分入力（6 桁中 2 桁）**の
まま Enter を送った（`scripts/research-ffw-osaka.mjs` の実験 A）。
...
`scripts/build-ffwtest-osaka.mjs` で `ASAOLIB/FFWDSPF` + `FFWPGM` を作り、
`scripts/research-ffw-osaka.mjs` が**生データストリームを core を通さず独立にパース**して採取した。
...
（この切り分けは `build-ffwtest-osaka.mjs` にそのまま残してある）
